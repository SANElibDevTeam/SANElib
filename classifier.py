   ...
   # TODO develop an algorithm to optimize the hyper parameters
    ...
    def visualize1D(self, feature1, target):
        ...
        if feature1 in self.numFeatures:
            ...
            num = self.executeQuery('Discretization 1d Numerical Histogram', '''
            ...
            # Categorical
            cat = self.executeQuery('Discretization 1d Categorical Histogram', '''
    ...
    def visualize2D(self, numFeat, catFeat, target):
        ...
        if numFeat in self.numFeatures and catFeat in self.catFeatures:
            ...
            multi = self.executeQuery('2d Discretization Histogram Estimation', '''
                        select distinct {} as xq, {} as mn, {} as mx, 
                        ({}+{})/2 as x_,
                        concat({}, ': ]', {}, ',', {}, ']') as bin,
                         {} as xc,  
